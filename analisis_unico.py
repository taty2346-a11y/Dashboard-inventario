...
# --- INYECCIÓN DE ESTILOS CORPORPORATIVOS ---
st.markdown("""
<style>
    h1, h2, h3 {
...
if archivo_carga:
    ...
    if st.sidebar.button("📊 Ejecutar Comparativa"):
        if sku_col not in df.columns:
            ...
            # --- CÁLCULO LOST / FOUND ---
            total_lost = abs(df[df['Diferencia_Uds'] < 0]['Diferencia_Uds'].sum())
            total_found = df[df['Diferencia_Uds'] > 0]['Diferencia_Uds'].sum()
            total_mov = total_lost + total_found
            
            pct_lost = (total_lost / total_mov) * 100 if total_mov > 0 else 0
            pct_found = (total_found / total_mov) * 100 if total_mov > 0 else 0
            ...
            st.markdown("#### 🎯 Distribución de LOST / FOUND")
            p1, p2, p3 = st.columns(3)

            p1.metric("📉 % LOST (Faltantes)", f"{pct_lost:.1f}%")
            p2.metric("📈 % FOUND (Sobrantes)", f"{pct_found:.1f}%")
            p3.metric("🔄 Total Movimientos", f"{total_mov:,}")

            # --- RESTO DEL CÓDIGO (GRÁFICOS, TABLAS, EXPORTACIÓN) ---
            # (SIN CAMBIOS)
