...
if archivo:
    ...
    # --- CRUCES DE TALLAS Y REUBICADOS (misma lógica) ---
    st.write("---")
    st.subheader("🏷️ Cruces de Variantes y Reubicados (solo si compensan)")
    ...
    for (ubic, raiz), grupo in df.groupby(["Ubicacion", "Raiz"]):
        ...
        if grupo["SKU"].nunique() > 1:
            ...
            if resto == 0:
                ...
                # NO compensa → RESTO a diferencias reales
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        pd.DataFrame(cruces_final).to_excel(writer, index=False, sheet_name="Cruces y Reubicados")
