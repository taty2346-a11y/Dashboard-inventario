...
if archivo:
    ...
    # Unidades informativas reubicados
    reubicados_units = int(df[df["SKU"].isin(reubicados_skus)]["Dif_Abs"].sum())

    # Cruces de talla SOLO si compensan LOST y FOUND
    cruces_comp_indices = []
    for (ubic, raiz), grupo in df.groupby(["Ubicacion", "Raiz"]):
        if grupo["SKU"].nunique() > 1:
            ...
            if resto == 0:
                cruces_comp_indices.append((ubic, raiz))

    cruces_units = int(
        df[df.set_index(["Ubicacion", "Raiz"]).index.isin(cruces_comp_indices)]["Dif_Abs"].sum()
    )
    ...
    st.write("---")
    st.subheader("🏷️ Cruces de Variantes (solo si compensan LOST/FOUND)")
    ...
    for (ubic, raiz), grupo in df.groupby(["Ubicacion", "Raiz"]):
        if grupo["SKU"].nunique() > 1:
            ...
            resto = grupo_dif["Diferencia"].sum()
            if resto != 0:
                # no compensa: las diferencias ya están en resumen_real
                continue
